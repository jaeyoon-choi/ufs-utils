...
/*
 * Fixed format sense keeps the key at byte 2 and the codes at 12 and 13,
 * the same bytes the Linux path reads.
 */
		      ...
		      __u8 cmd_len, __u32 byte_cnt, int dir)
{
	...
	for (tries = 0; tries < 2; tries++) {
		...
		if ((sense[2] & 0x0f) != SENSE_KEY_UNIT_ATTENTION)
			break;
	}
	...
	if ((ccb.ccb_h.status & CAM_AUTOSNS_VALID) != 0) {
		...
		fprintf(stderr, "  senseKey %s, asc 0x%02x, ascq 0x%02x\n",
