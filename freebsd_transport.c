		...
		__u8 cmd_len, __u32 byte_cnt, int dir)
{
	...
	for (tries = 0; tries < 2; tries++) {
		 ...
		 *
		 * CAM_DEV_QFRZDIS has the kernel thaw the device queue a
		 * CHECK CONDITION froze, since userland cannot.
		 */
		cam_fill_csio(&ccb.csio, 1, NULL, ccb_flags | CAM_DEV_QFRZDIS,
		    MSG_SIMPLE_Q_TAG,
...
_Static_assert(sizeof(struct ufs_ehs) == ARPMB_EHS_UNITS * 32,
	       "EHS length is counted in 32 byte units");
		...
		__u32 data_buf_len, __u8 *data_buf, bool write)
{
	...
	/*
	 * The caller reads the RPMB result out of the response EHS, so a
	 * response without one is an error, not a success.
	 */
	if (pt.resp_upiu.header.ehs_length != ARPMB_EHS_UNITS) {
		print_error("%s: response is missing the advanced RPMB EHS",
		    __func__);
		rpmb_rsp->bsg_reply.result = -EIO;
		return -EIO;
	}
	ehs = (uint8_t *)&pt.resp_upiu + sizeof(struct utp_upiu_req);
	memcpy(&rpmb_rsp->ehs_rsp, ehs, sizeof(struct ufs_ehs));

	if (pt.xfer_len > data_buf_len) {
		print_error("%s: device returned %u bytes, the buffer holds %u",
		    __func__, pt.xfer_len, data_buf_len);
		rpmb_rsp->bsg_reply.result = -EIO;
		return -EIO;
	}
